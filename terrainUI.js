...
class TerrainUI {
    constructor(canvas, params) {
        ...
        this.visualizePointsSphere = function() {
            ...
            ctx.strokeStyle = "#000";
            ctx.stroke();
            var randomPoint = { 'type': 'MultiPoint', 'coordinates': this.terrain.points };
            ctx.beginPath();
            path(randomPoint);
            ctx.stroke();
        };
        ...
        this.visualizeTrianglesColor = function() {
            ...
            var mappedvals = this.terrain.heightMap.map(function(x) { return x > hi ? 1 : x < lo ? 0 : (x - lo) / (hi - lo); });
            for (var f in this.terrain.edges) {
                ...
                ctx.beginPath();
                var a = { 'type': 'LineString', 'coordinates': [start, stop] };
                path(a);
